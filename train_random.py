...
train_dataset = concatenate_datasets(ordered_train_datasets).shuffle(seed=SEED)
...
output_name = f"qwen2.5-3B-random-{timestamp}"
wandb_name = f"global-random-sampling-{timestamp}"

wandb.init(
    project="qwen-constraint-finetune",     
    name=wandb_name,
    tags=["qwen2.5-3b", "a100", "random-sampling"] 
)

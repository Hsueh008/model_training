...
for name in category_names:
    ...
    stage_ds = train_ds.shuffle(seed=SEED)
...
sorted_metadata = sorted(dataset_metadata_list, key=lambda x: (x["level"], x["score"]))

print("\n[Info] 最終訓練順序 (Level-Up CL):")
for i, meta in enumerate(sorted_metadata):
    print(f"  階段 {i+1:02d} | Level: {meta['level']} | Score: {meta['score']:.3f} | {meta['name']}")

ordered_train_datasets = [meta["dataset"] for meta in sorted_metadata]
